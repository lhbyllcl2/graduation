  ...
  <div>
    <section v-if="!showLoading" class="shop_container">
      <header class="shop_detail_header" ref="shopheader" :style="{zIndex: showActivities? '14':'10'}">
        <img :src="shopDetailData.image_path" class="header_cover_img">
        <section class="description_header">
          <router-link to="/shop/shopDetail" class="description_top">
            <section class="description_left">
              <img :src="shopDetailData.image_path">
            </section>
        ...
        <section v-show="changeShowType =='food'" class="food_container">
            ...
            <section class="menu_left" id="wrapper_menu" ref="wrapperMenu">
              <ul>
                <li v-for="(item,index) in menuList" :key="index"
                    :class="index == menuIndex?'menu_left_li activity_menu':'menu_left_li'" @click="chooseMenu(index)">
                          ...
                          <ul v-if="foods.attributes.length" class="attributes_ul">
                            <li v-if="attribute" v-for="(attribute, foodindex) in foods.attributes" :key="foodindex"
                              ...
                              <p :style="{color: attribute.icon_name == '新'? '#fff' : '#' + attribute.icon_color}">
                                {{attribute.icon_name == ''? '新品':attribute.icon_name}}</p>
                            </li>
                          </ul>
          ...
          <section class="buy_cart_container">
            ...
            <section class="gotopay" :class="{gotopay_acitvity: minimumOrderAmount <= 0}">
              ...
              <span class="gotopay_button_style" @click="goToConfirmOrder" v-else>
                去结算
              </span>
            </section>
          </section>
              ...
              <ul class="rating_list_ul">
                <li v-for="(item, index) in ratingList" :key="index" class="rating_list_li">
                  ...
                  <section class="rating_list_details">
                    ...
                    <div>
                      <p style="font-size: 0.5rem;color: #666;padding: 0.4rem 0">{{item.rating_text}}</p>
                      <p style="font-size: 0.5rem;color: #666;background:#F5F5F5;padding: 0.4rem" v-if="item.reply_contents"><span style="color: #3190e8">商家回复：</span>{{item.reply_contents}}</p>
                    </div>
...
<script>
  ...
  import {shopDetails, foodMenu, getRatingList, ratingScores, ratingTags} from 'src/service/getData'
  ...
  export default {
    ...
    computed: {
      ...
      minimumOrderAmount: function () {
        if (this.shopDetailData) {
          return Numeral(this.shopDetailData.float_minimum_order_amount - this.totalPrice).format('0.0');
        } else {
          return null;
        }
      },
    ...
    methods: {
      ...
      async initData() {
        ...
        //评论列表
        await getRatingList(this.shopId).then(res => {
      ...
      chooseMenu(index) {
        ...
        this.foodScroll.scrollTo(0, -this.shopListTop[index], 300);
