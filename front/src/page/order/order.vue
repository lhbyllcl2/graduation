      ...
      <li class="order_list_li" v-for="item in orderList" :key="item.id">
        ...
        <section class="order_item_right">
          ...
          <div class="order_again">
            <router-link :to="{path: '/evaluate', query: {order_sn: item.order_sn}}" tag="span" class="buy_again" v-if="item.status==3">去评价</router-link>
          </div>
        </section>
      </li>
    </ul>
  ...
  export default {
    ...
    methods: {
      ...
      showDetail(item) {
        ...
        this.$router.push({path: '/order/orderDetail', query: {order_id: item.order_sn}});
      },
