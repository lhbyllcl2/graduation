    ...
    <section v-if="!showLoading">
      ...
      <section class="food_list">
        <header v-if="checkoutData.cart.restaurant_info">
          <img :src="checkoutData.cart.restaurant_info.image_path">
  ...
  export default {
    ...
    methods: {
      ...
      async initData() {
        ...
        //检验订单是否满足条件
        await checkout([newArr], this.shopId).then(res => {
      ...
      async initAddress() {
        await getAddressList().then(res => {
          if (res.code === 200 && res.data.length) {
            this.CHOOSE_ADDRESS({address: res.data[0], index: 0});
          }
        });
      },
      //显示付款方式
      ...
      async confrimOrder() {
        ...
        //发送订单信息
        await placeOrders(this.checkoutData.cart.id, this.choosedAddress.id, this.remarklist, this.checkoutData.cart.groups).then(res => {
          if (res.code === 200) {
            this.ORDER_SUCCESS(res.data);
            this.$router.push({path: '/confirmOrder/payment', query: {order_id: res.data.order_id}});
          } else {
            this.showAlert = true;
