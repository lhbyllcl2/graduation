    ...
    <section class="page_text_container">
      ...
      <section class="section_list">
        ...
        <section class="section_right">
          ...
          <input v-if="phone_bk" type="text" name="anntherPhoneNumber" placeholder="备选电话" v-model="anntherPhoneNumber"
  ...
  export default {
    ...
    methods: {
      ...
      async addAddress() {
        ...
        postAddAddress(this.searchAddress.name, this.address_detail, this.searchAddress.location, this.name, this.phone, this.anntherPhoneNumber,this.sex, this.tag).then(res => {
          //保存成功返沪上一页，否则弹出提示框
          if (res.code !== 200) {
            this.showAlert = true;
