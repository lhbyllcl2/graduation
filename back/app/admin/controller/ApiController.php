...
class ApiController extends AdminCheckLogin
{
    ...
    public function upload()
    {
        ...
        $aliyun = AliYunOss::getInstance();
        $uploadRes = $aliyun->uploadFile(AliYunOss::getBucketName(), $fileName, $file->getInfo()['tmp_name']);
        if (is_array($uploadRes)) {
            return vae_assign(1, '', $uploadRes['info']['url']);
        }
        return vae_assign(0, '', '上传失败');
    }
